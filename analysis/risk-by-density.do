...
collapse (first) employment_density (mean) communication_share infection_share [fw=employment], by(zip)

*drop rural areas, CBP is not representative for these
drop if employment_density<10

generate ln_density = ln(employment_density)
foreach X of var *_share {
	lowess `X' ln_density, bw(0.5) generate(`X'_hat)
}

line communication_share_hat infection_share_hat employment_density, sort ///
ytitle("Average occupation index of industries") xtitle("Employment density (person/km2, log scale)") ///
